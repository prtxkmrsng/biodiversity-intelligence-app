...
export class MLPipeline {
  ...
  async predict(imageUri: string, base64Data?: string): Promise<Observation> {
    ...
    const maxLogit = Math.max(...(Array.from(outputValues) as number[]));
    const exps = (Array.from(outputValues) as number[]).map(x => Math.exp(x - maxLogit));
    const sumExps = exps.reduce((a, b) => a + b, 0);
    const probabilities: number[] = exps.map((e: number) => e / sumExps);
